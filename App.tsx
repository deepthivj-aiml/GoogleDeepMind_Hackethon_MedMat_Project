...
import { generateDesign, generateCadVisualization, validateDeviceInput, getDeviceDefaults } from './services/gemini';
...
const InteractiveBioCore = () => {
  ...
  useEffect(() => {
    ...
    // Create a complex "Molecular Lattice" geometry for a professional medical look
...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center py-12 px-4 md:px-8 ${isPrintMode ? 'print:p-0' : ''}`}>
      ...
      {/* Professional Hero Section & Description */}
      <div className="w-full max-w-[1500px] mb-20 flex flex-col lg:flex-row items-center justify-between gap-16 print:hidden animate-in fade-in duration-1000">
        <div className="flex-1 space-y-8 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-5 py-2 bg-white/50 backdrop-blur-md rounded-full shadow-sm border border-slate-200">
            ...
            <span className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Next-Gen Clinical Synthesis Engine</span>
          </div>
      ...
      <div className="w-full max-w-[1500px] space-y-8 print:hidden">
        {/* DO NOT CHANGE INPUT/OUTPUT UI BELOW THIS LINE */}
        {error && (
        ...
        <section className="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(15,23,42,0.08)] p-10 border border-slate-100 relative overflow-hidden transition-all hover:shadow-[0_48px_80px_-24px_rgba(15,23,42,0.12)]">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            {/* Left Column: ID and Device Specs */}
            ...
            {/* Right Group: Material Specification Protocol */}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
              <div className="xl:col-span-5 space-y-6">
                ...
                <div className="flex-grow pt-2">
                  {resultsSubTab === 'MATRIX' ? <MaterialAnalysis materials={design.materials} enabledRequirements={requirements} groundingSources={design.groundingSources} /> : <div className="bg-white rounded-[1.5rem] shadow-xl border border-slate-100 p-6 min-h-[600px]"><SDSViewer sdsLibrary={design.sdsLibrary} deviceName={design.deviceName} onPrintAll={() => handleExportFile('SDS_REPORT')} /></div>}
                </div>
              </div>
